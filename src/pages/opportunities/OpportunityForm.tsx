...
import { CURRENCY_CODES, HTTP_METHODS } from '../../utils/Constants'
...
export function OpportunityForm ({ state, httpReqMethod }: any) {
  ...
  const submitForm = () => {
    ...
    const data = {
      ...
      assigned_to: formData.assigned_to.map((profile: any) => profile.id),
    ...
    const apiUrl = httpReqMethod === HTTP_METHODS.PUT ? `${OpportunityUrl}/${state?.id}/` : `${OpportunityUrl}/`
    fetchData(apiUrl, httpReqMethod, JSON.stringify(data), Header)
      .then((res: any) => {
        if (!res.error) {
          resetForm()
