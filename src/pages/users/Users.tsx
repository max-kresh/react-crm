import React, { SyntheticEvent, useDebugValue, useEffect, useState, useContext } from 'react'
...
import { fetchData, fetchRawData, compileHeader } from '../../components/FetchData'
...
export default function Users () {
  ...
  const onDelete = (id: any) => {
    fetchData(`${UsersUrl}/${id}/`, 'delete', null as any, compileHeader())
      .then((data) => {
        if (!data.error) {
          getUsers()
          setIsDelete(false)
  ...
  const DeleteItem = () => {
    fetchData(`${UserUrl}/${selectedId}/`, 'DELETE', null as any, compileHeader())
      .then((res: any) => {
  ...
  let roleBasedHeadCells = [...headCells]
  if (!userCtx.isAdmin()) {
    roleBasedHeadCells = roleBasedHeadCells.filter((header) => header.id !== 'actions')
  }
                      ...
                      stableSort(
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell
                              className={userCtx.isAdmin() ? 'tableCell-link' : ''}
                              onClick={userCtx.isAdmin() ? () => userDetail(item.id) : () => {}}
                            >
                            ...
                            {userCtx.isAdmin() && <TableCell className="tableCell">
                              <IconButton>
                                <FaEdit
